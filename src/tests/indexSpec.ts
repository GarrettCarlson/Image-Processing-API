...
describe('Image resizer middleware spec', function () {
  ...
  it('should resize the image to the given height and width', async () => {
    const newHeight = 500;
    const newWidth = 500;
    await resizeImage(testImageName, newHeight, newWidth);
    const newFileName = path.resolve(
      './api/thumb/' + testImageName.replace('.jpg', '') + '_500_500.jpg'
    );
...
describe('Endpoint spec', function () {
  it('should open in the browser with status 200', async () => {
    const res = await req.get(
      '/api/images?filename=' + testImageName + '&height=100&width=100'
    );
  ...
  it('should provide an error to the user if the image requested does not exist', async () => {
    const response = await req.get(
      '/api/images?filename=' + dneImageName + '&height=100&width=100'
    );
    expect(response.text).toMatch(
      /(Error: Image requested .* does not exist.)/
    );
  });
